...
async function verifyDiagnosticReport(page) {
  ...
  const button = report.samples.find((sample) => sample.view === "diary-view" && !sample.ui.modalOpen && sample.layout.addButton?.centerHitTest.hitsButton)?.layout.addButton;
  assert.ok(button, "A recording must hit the real diary button or its icon at the measured center");
  assert.equal(button.hidden, false);
  assert.equal(button.hiddenAncestor, false);
  ...
  assert.equal(button.centerHitTest.target.surface, "main");
  assert.ok(["BUTTON", "SPAN"].includes(button.centerHitTest.target.tagName));
  const covered = report.samples.find((sample) => sample.ui.modalOpen && sample.layout.addButton?.centerHitTest.target?.surface === "dialog")?.layout.addButton;
  assert.ok(covered, "The top-layer modal must be detected over the button even when the button's own CSS remains visible");
  assert.equal(covered.centerHitTest.hitsButton, false);
  assert.equal(covered.visibility, "visible");
  const hidden = report.samples.find((sample) => sample.view === "directories-view")?.layout.addButton;
  assert.ok(hidden, "Button diagnostics must also explain an inactive diary view");
  assert.equal(hidden.hidden, false);
  assert.equal(hidden.hiddenAncestor, true);
  assert.equal(hidden.centerHitTest.status, "no-box");
  const events = report.samples.flatMap((sample) => sample.inputEvents);
  assert.ok(events.some((event) => event.type === "touchstart" && event.isTrusted && event.addButtonAtCapture?.centerHitTest.hitsButton), "Native taps must retain the button state from the capture phase");
