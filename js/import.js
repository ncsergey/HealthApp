...
function oneDecimalInRange(value, min, max) { return Number.isFinite(value) && Math.abs(value * 10 - Math.round(value * 10)) < 1e-9 && value >= min && value <= max; }
...
function validateProfile(profile) {
  ...
  if (!oneDecimalInRange(profile.heightCm, 50, 300)) throw new Error("Рост в профиле должен быть от 50 до 300 см и содержать не более одного знака после точки.");
...
export async function parseBackupFile(file) {
  ...
  if (raw.version >= 10 && !isValidGlassBlurIntensity(raw.settings.glassBlurIntensity)) throw new Error("Некорректная интенсивность размытия Liquid Glass в резервной копии.");
